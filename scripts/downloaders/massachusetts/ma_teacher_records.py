...
========================================================
Downloader for Massachusetts educator / teacher records data.

Downloads teacher data and licensure reports from MA DOE profiles site
using ASP.NET form POST export.

Saves to: data/massachusetts/teacher_staff/
"""
...
logger = setup_logger("downloader.massachusetts.teacher_staff")

STATE = "massachusetts"
CATEGORY = "teacher_staff"

TEACHER_ENDPOINTS = [
    # Educator counts by district
    ("https://profiles.doe.mass.edu/statereport/teacherdata.aspx", "teacherdata"),
    # Teacher salaries
    ("https://profiles.doe.mass.edu/statereport/teachersalaries.aspx", "teachersalaries"),
]
...
class MassachusettsTeacherRecordsDownloader:
    """
    Download MA DOE teacher/educator data.
    ...
    def download_all(self) -> None:
        """Download all MA teacher/educator data files."""
        self.logger.info("Starting Massachusetts teacher staff download.")
        total = 0
        for url, label in TEACHER_ENDPOINTS:
            files = _post_export(self.session, url, self.output_dir, label)
            total += len(files)
            time.sleep(1.5)
        self.logger.info("Massachusetts teacher staff complete. Files downloaded: %d", total)


if __name__ == "__main__":
    MassachusettsTeacherRecordsDownloader().download_all()
