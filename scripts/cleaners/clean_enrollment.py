"""
scripts/cleaners/clean_enrollment.py
======================================
Cleaner for enrollment data — works for both Nevada and Massachusetts.

Reads every file in data/{state}/raw/enrollment/, normalises the data,
and saves cleaned CSVs to data/{state}/cleaned/enrollment/.
...
4. Strip commas from enrollment count columns (e.g. "1,234" → "1234").
...
logger = setup_logger("cleaner.enrollment")
...
def _strip_number_formatting(df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove comma thousands-separators from string columns that contain
    numeric enrollment counts (e.g. "12,345" → "12345").
    """
    for col in df.select_dtypes(include="object").columns:
        df[col] = df[col].str.replace(r"^(\d{1,3}(,\d{3})+)$", lambda m: m.group(0).replace(",", ""), regex=True)
    return df


def _read_file(path: Path) -> pd.DataFrame:
    """Load a raw enrollment file into a DataFrame."""
...
def _clean_dataframe(df: pd.DataFrame, state: str, source_file: str) -> pd.DataFrame:
    """Apply standard cleaning transformations to an enrollment DataFrame."""
    df = _standardise_columns(df)
    df = df.dropna(how="all").dropna(axis=1, how="all")
    df = _strip_number_formatting(df)
    df["state"] = state
...
def clean(state: str) -> None:
    """
    Clean all raw enrollment files for the given state.
    ...
    if not raw_files:
        logger.warning("No raw enrollment files found for %s in %s", state, raw_path)
        return

    logger.info(
        "Cleaning %d enrollment file(s) for %s → %s",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Clean raw enrollment files for a given state."
    )
