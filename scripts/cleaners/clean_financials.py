"""
scripts/cleaners/clean_financials.py
=====================================
Cleaner for financial data — works for both Nevada and Massachusetts.

Reads every file in data/{state}/raw/financials/, normalises the data,
and saves cleaned CSVs to data/{state}/cleaned/financials/.
...
4. Strip dollar signs, commas, and percent signs from numeric-looking columns.
...
logger = setup_logger("cleaner.financials")
...
def _strip_currency_formatting(df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove dollar signs, commas, and percent signs from column values
    that appear to contain financial figures.

    Only string-typed columns are modified to avoid corrupting already-numeric data.
    """
...
def _read_file(path: Path) -> pd.DataFrame:
    """Load a raw financial file into a DataFrame."""
...
def _clean_dataframe(df: pd.DataFrame, state: str, source_file: str) -> pd.DataFrame:
    """Apply standard cleaning transformations to a financial DataFrame."""
    df = _standardise_columns(df)
    df = df.dropna(how="all").dropna(axis=1, how="all")
    df = _strip_currency_formatting(df)
    df["state"] = state
...
def clean(state: str) -> None:
    """
    Clean all raw financial files for the given state.
    ...
    if not raw_files:
        logger.warning("No raw financial files found for %s in %s", state, raw_path)
        return

    logger.info(
        "Cleaning %d financial file(s) for %s → %s",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Clean raw financial files for a given state."
    )
